...
const inviteStaff = async (email) => { 
    ...
    await dB.organizationInvites.create({email, token: token.token})
    return emailService.sendInvite(email, token, organization.name) // When email configuration is done it will be available
}
